...
Konstruktor = [
	...
	"fungsi",     # function
	"fomo",       # while
	"udahan",     # end (menutup blok statement)
	"kembali",    # return
	"lanjutkan",  # continue
	"berhenti"    # break
]
	...
	def buatToken(self):
		...
		while self.karakterSkrg != None:
			if self.karakterSkrg in " \t":
				...
				tokens.append(self._daftarliterally())
			elif self.karakterSkrg in '"\'':
...
class NodeAksesliterally:
	def __init__(self, namaliterally_token):
		self.nama_token_literally = namaliterally_token

		self.posisi_awal = self.nama_token_literally.posisi_awal
		self.posisi_akhir = self.nama_token_literally.posisi_akhir


class NodeBuatliterally:
	def __init__(self, namaliterally_token, isi_node):
		self.nama_token_literally = namaliterally_token
...
class NodeFor:
	def __init__(
		...
		self.namaliterally_token = namaliterally_token
		...
		self.posisi_awal = self.namaliterally_token.posisi_awal
...
#literally b = 0; fomo b < 15 maka tulis(b); literally b = b + 1 tutup
class NodeBuatFungsi:
	def __init__(self, namaliterally_token, nama_parameter_token, isi_node, harus_return_null):
		self.namaliterally_token = namaliterally_token
		self.nama_parameter_token = nama_parameter_token
		self.isi_node = isi_node
		self.harus_return_null = harus_return_null

		if self.namaliterally_token:
			self.posisi_awal = self.namaliterally_token.posisi_awal
...
class Parser:
			...
			if res.error:
				return res.gagal(
					SintaksSalah(
						self.tokenSkrg.posisi_awal,
						self.tokenSkrg.posisi_akhir,
						"Dibutuhkan ')', 'literally', 'kalo', 'untuk', 'fomo', 'fungsi', int, float, nama variabel, '+', '-', '(' atau 'bukan'",
					)
				)
	...
	def if_expr(self):
		res = HasilParse()
		all_cases = res.daftar(self.if_expr_cases('kalo'))
	...
	def if_expr_c(self):
		...
		if self.tokenSkrg.sama_dengan(TokenKeyword, 'jikatidak'):
			...
			if self.tokenSkrg.tipe == TokenLineBaru:
				...
				if self.tokenSkrg.sama_dengan(TokenKeyword, 'udahan'):
					res.daftar_kemajuan()
					self.maju()
				else:
	...
	def if_expr_cases(self, case_keyword):
		...
		if self.tokenSkrg.tipe == TokenLineBaru:
			...
			if not self.tokenSkrg.sama_dengan(TokenKeyword, 'udahan'):
				return res.gagal(SintaksSalah(
					self.tokenSkrg.posisi_awal, self.tokenSkrg.posisi_akhir,
					f"Dibutuhkan 'udahan'"
				))
	...
	def for_expr(self):
		...
		if self.tokenSkrg.tipe == TokenLineBaru:
			...
			return res.berhasil(NodeFor(nama_literally, isi_awal, isi_akhir, isi_step, isi, True))
		
		isi = res.daftar(self.statement())
		if res.error: return res

		return res.berhasil(NodeFor(nama_literally, isi_awal, isi_akhir, isi_step, isi, False))

	def while_expr(self):
		res = HasilParse()

		if not self.tokenSkrg.sama_dengan(TokenKeyword, "fomo"):  # while
			return res.gagal(
				SintaksSalah(
					self.tokenSkrg.posisi_awal,
					self.tokenSkrg.posisi_akhir,
	...
	def atom(self):
		res = HasilParse()
		tokenskrg = self.tokenSkrg#fomo ae < 10 maka literally ae = ae + 1 tulis(ae);
		...
		if tokenskrg.tipe in (TokenInteger, TokenFloat):
			...
			res.daftar_kemajuan()
			self.maju()
			return res.berhasil(NodeAksesliterally(tokenskrg))
		...
		elif tokenskrg.sama_dengan(TokenKeyword, "kalo"):
		...
		elif tokenskrg.sama_dengan(TokenKeyword, "fomo"):
		...
		return res.gagal(
			SintaksSalah(
				...
				"Harus memiliki int, float, literally, '+', '-' atau parentesis",
			)
		)
		...
		if res.error:
			return res.gagal(
				...
				or SintaksSalah(
					self.tokenSkrg.posisi_awal,
					self.tokenSkrg.posisi_akhir,
					"Harus memiliki int, float, literally, '+', '-', 'bukan', atau parentesis",
				)
			)
		...
		if res.error:
			return res.gagal(
				SintaksSalah(
					...
					self.tokenSkrg.posisi_akhir,
					res.error.details
					or "Harus memiliki lanjutkan, kembali, berhenti, int, float, literally, '+', '-' atau parentesis",
				)
			)
	...
	def expr(self):
		...
		if self.tokenSkrg.sama_dengan(TokenKeyword, "literally"):
			res.daftar_kemajuan()
			self.maju()
			...
			if res.error:
				return res
			return res.berhasil(NodeBuatliterally(nama_literally, expr))
		...
		if res.error:
			return res.gagal(
				SintaksSalah(
					...
					self.tokenSkrg.posisi_akhir,
					res.error.details
					or "Harus memiliki int, float, literally, '+', '-' atau parentesis",
				)
			)
	...
	def buat_fungsi(self):
		...
		if self.tokenSkrg.tipe == TokenIdentifier:
			token_nama_literally = self.tokenSkrg
			res.daftar_kemajuan()
			self.maju()
			if self.tokenSkrg.tipe != TokenParentesisKiri:
				return res.gagal(
		...
		if self.tokenSkrg.tipe == TokenPanah:
			...
			return res.berhasil(
			   NodeBuatFungsi(token_nama_literally, nama_token_parameter, isifungsi, True)
			)
		...
		return res.berhasil(
			NodeBuatFungsi(token_nama_literally, nama_token_parameter, isistatement, False)
		)
...
class BaseFungsi(Isi):
	...
	def hitung_parameter(self, nama_parameters, parameters, konteks):
		for i in range(len(parameters)):
			...
			konteks.TabelSimbol.spill(nama_parameter, isi_parameter)
...
class BuiltInFungsi(BaseFungsi):
	...
	def esekusi_tunggu(self, konteks_esekusi):
		...
		return HasilRuntime().berhasil(Angka.nil)
		# literally e = 0; fomo e < 50 maka tulis(e + 1); tunggu(1); literally e = e + 1 tutup
	...
	def esekusi_masuk(self, konteks_esekusi):
	...
	esekusi_masuk.nama_parameter = ["list", "value"]
...
BuiltInFungsi.spill = BuiltInFungsi("print")
...
class Interpreter:
	...
	def kunjungi_NodeAksesliterally(self, node, konteks):
		res = HasilRuntime()
		nama_literally = node.nama_token_literally.value
		isi = konteks.TabelSimbol.dapat(nama_literally)
		...
		if not isi:
			return res.gagal(
				RTError(
					node.posisi_awal,
					node.posisi_akhir,
					f"literally '{nama_literally}' tidak dideklarasikan",
					konteks,
				)
			)
	...
	def kunjungi_NodeBuatliterally(self, node, konteks):
		res = HasilRuntime()
		nama_literally = node.nama_token_literally.value
		...
		konteks.TabelSimbol.spill(nama_literally, isi)
		return res.berhasil(isi)
	...
	def kunjungi_NodeFor(self, node, konteks):
		...
		while condition():
			konteks.TabelSimbol.spill(node.namaliterally_token.value, Angka(i))
			i += step_value.nilai
	...
	def kunjungi_NodeBuatFungsi(self, node, konteks):
		...
		nama_fungsi = node.namaliterally_token.value if node.namaliterally_token else None
		...
		if node.namaliterally_token:
			konteks.TabelSimbol.spill(nama_fungsi, isi_fungsi)
...
global_tabel_simbol.spill("nil", Angka.nil)
global_tabel_simbol.spill("salah", Angka.salah)  # false
global_tabel_simbol.spill("benar", Angka.benar)  # true
global_tabel_simbol.spill("spill", BuiltInFungsi.spill)
global_tabel_simbol.spill("tunggu", BuiltInFungsi.wait)
global_tabel_simbol.spill("tipe", BuiltInFungsi.tipe)
global_tabel_simbol.spill("panjang", BuiltInFungsi.panjang)
global_tabel_simbol.spill("masuk", BuiltInFungsi.masuk)
global_tabel_simbol.spill("substring", BuiltInFungsi.substring)
global_tabel_simbol.spill("esekusi", BuiltInFungsi.esekusi_builtin)
